...
def createEpochsArray(EEG, info, tmin=0, add_events=None, event_id=None):
    '''
    Creates an MNE EpochsArray from a NumPy array. 
    If add_events contains an array with events/categories, these events are added to each trial in the EpochsArray.
    ...
    # Returns
        EEG: MNE EpochsArray structure
            EEG data as an EpochsArray.
...
def extractEvoked(epochsArray, add_events=None, event_id=None):
    '''
    Converts an MNE EpochsArray structure to an MNE Evoked structure.
    
    # Arguments
        epochsArray: MNE EpochsArray structure
        add_events: NumPy array
            Array with integers indicative of events/categories (same length as EEG.shape[0]).
        event_id: dictionary
            Dictionary containing event/category name as key, and integer as value. E.g. dict(scene=0, face=1).
    ...
    # Returns
        evokedArray: MNE Evoked structure
            EEG data averaged across trials, or averaged across events/conditions (if add_events and event_id provided).
