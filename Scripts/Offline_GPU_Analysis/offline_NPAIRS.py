#
# Author: Greta Tuckute, 24 June 2019
...
def analyzeOffline(subjID):
    ...
    for run in range(n_it*4):
        score_per_block[run] = metrics.accuracy_score(y_test_feedback[run*50:(run+1)*50], y_pred_test1[run*50:(run+1)*50]) 
        a_per_block[run] = (len(np.where((np.array(alpha_test[j:j+50])>0.5))[0])/50)
        j += 50
