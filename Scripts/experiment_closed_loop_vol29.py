...
# for debugging
#info2 = StreamInfo('alphaStream', 'Markers', 1, 0, 'int32', 'myuidw43536')
#outlet2 = StreamOutlet(info2)

############### Experimental initialization ###############

# Initializing window
# win = visual.Window(size=[1910, 1070], fullscr=False,winType='pyglet') #GPU

win = visual.Window(size=[1910, 1070], fullscr=True,winType='pyglet',allowGUI=True,screen=1,units='pix') #GPU
#win = visual.Window(size=[910, 170], fullscr=False,winType='pyglet',allowGUI=True,screen=0,units='pix') #GPU

# win = visual.Window(size=[700, 400], fullscr=False,winType='pyglet',allowGUI=True,screen=1,units='pix') #GPU



#win = visual.Window(size=[1000, 2000], fullscr=False,screen=0,
#    allowGUI=False, allowStencil=False,
#    monitor='testMonitor', color=[0, 0, 0], colorSpace='rgb') #GT laptop
...
def saveAlphaStableDataFrame(subjID):
    """
    
    """
    df_alphaStableLst = pd.DataFrame(alphaStableSave)
    df_alphaStableLst.to_csv(subject_path + '\\' + subjID + '\\alphaStable_subjID_' + str(subjID) + '.csv')
...
#        subj_orig=subjID
    ...
    if control[0] == '1':
        ...
        for run in list(range(0,numRuns)):
            ...
            #alphamarker = inlet_alpha.pull_sample(timeout=0.01)
