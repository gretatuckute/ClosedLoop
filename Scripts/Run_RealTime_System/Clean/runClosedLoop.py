...
info_fs500 = createInfoMNE(reject_ch=0, sfreq=fs) 
info_fs100 = createInfoMNE(reject_ch=1, sfreq=fs_new)

# Clear EEG and experiment streams to ensure a clean start
clear_stream(inlet_marker)
clear_stream(inlet_EEG)

n_runs = 6 # No. runs (total)
...
while marker[0]+1 < n_trials:
    ...
    if state == 'stable':
        ...
        stable_blocksSSP = averageStable(stable_blocksSSP)
